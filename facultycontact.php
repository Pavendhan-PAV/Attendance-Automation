...
<body>
                ...
                if(isset($_SESSION['parentuser']))
  {
    $sid=$_POST['sid'];
    $_SESSION['sid']=$sid;
    $display="SELECT * from course where cno in (SELECT cno from sidcno where sid='$sid') ";
?>
<h3><?php  if(isset($_SESSION['MAILERROR'])){ echo($_SESSION['MAILERROR']."\n");unset($_SESSION['MAILERROR']);}     ?></h3>
		<form action=intermediate_mailer.php method="POST">
        ...
        <p>
        <div></div>
  			<textarea id="deta" placeholder="Message" rows="3" cols="15" style="text-transform: capitalize;
        padding-left: 10px; font-size: 1em; border: none;opacity: 0.7;max-height: 5em;border-radius: 0.25em;max-width: 20em;min-width: 15em; color: black;" name="content"></textarea>
  			<?php
	...
	{
		$email=$_SESSION['studentuser'];
		$display="SELECT * from course where cno in (SELECT cno from sidcno where sid=(SELECT sid from student where smailid='$email' )) ";
?>
  <h3><?php  if(isset($_SESSION['MAILERROR'])){ echo($_SESSION['MAILERROR']."\n");unset($_SESSION['MAILERROR']);}     ?></h3>
<form action=intermediate_mailer.php method="POST">

			<select id="deta" required="" style="color: #1b1b1b;  font-size: 1em; max-width: 20em;min-width: 15em;border: none;opacity: 0.7;min-height: 3.25em;border-radius: 0.25em; padding-left: 3.7em; " name="cno">
...
?>      </select> <p><div></div>
  			<textarea id="new" placeholder="Message" rows="3" cols="15" style="text-transform: capitalize;padding-left: 10px; font-size: 1em; border: none;opacity: 0.7;max-height: 5em;border-radius: 0.25em;max-width: 20em;min-width: 15em; color: black;"" name="content"></textarea>
...
else
{
  die("<h2 id='deta'>Please Login!</h2><p>");
  	...
  	<p><div></div>
    <input class="btn btn-success btn-block" type="submit" id="register" name="create" value="SEND" style="max-width: 12em;">
  </form>
